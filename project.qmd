...
dat <- raw %>%
  transmute(
    yr  = as.integer(zap_labels(.data[[yrv]])),
    y   = as.integer(zap_labels(.data[[hapv]])),
    age = as.numeric(zap_labels(.data[[agev]])),
    wrk = as.integer(zap_labels(.data[[wrkv]])),
    inc = as.numeric(zap_labels(.data[[incv]])),
    wt  = if (!is.na(wtv)) as.numeric(zap_labels(.data[[wtv]])) else 1 ) %>%
...
fig_dat <- prop.table(
  ...
  as.data.frame() |>
  transmute(
    period  = if_else(post == 0, "2018 (Pre)", "2021 (Post)"),
...
fig1 <- ggplot(fig_dat, aes(x = period, y = p, fill = outcome)) +
  ...
  labs(
    ...
    y = "Weighted share",
    fill = NULL,
    title = "Weighted happiness distribution (GSS 2018 vs 2021)"  ) +
...
# 8 Table 1: Regression results (Odds Ratios + 95% CI)
#    (Printed inside your Rmd)
...
knitr::kable(
  tab1,
  caption = "Table 1. Survey-weighted logistic regression (odds ratios). Outcome: Happy = 1 (Very/Pretty happy), 0 (Not too happy).",
  align = "lrrrr"
)

# (Optional) also save the table as a .tex file for LaTeX workflows
writeLines(
  knitr::kable(tab1, format = "latex", booktabs = TRUE,
               caption = "Survey-weighted logistic regression (odds ratios).",
               align = "lrrrr"),
  "output/Table1_logit_OR_2018_2021.tex"
)

# (Optional) save CSV too
readr::write_csv(tab1, "output/Table1_logit_OR_2018_2021.csv")
